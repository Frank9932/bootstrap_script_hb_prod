...
#   06_docker_tools.sh  (Docker CE + Compose + CLI tools)
#
# Usage:
...
This orchestrator will run the following modules:
  ...
  06_docker_tools.sh    - Install Docker CE + Compose + CLI tools
...
BANNER
  ...
  log "Step 1/6: Running 01_root_password.sh (emergency root password)..."
  ...
  log "Step 2/6: Running 02_create_user.sh (non-root user + SSH key)..."
  ...
  log "Step 3/6: Running 03_harden_sshd.sh (disable root + password auth)..."
...
INFO_SSH
  ...
  log "Step 4/6: Running 04_firewall.sh (firewalld baseline: SSH only)..."
...
INFO_FW
  ...
  log "Step 5/6: Running 05_fail2ban.sh (SSH brute-force protection)..."
...
INFO_F2B
  ...
  log "Step 6/6: Running 06_docker_tools.sh (Docker + CLI tools)..."
